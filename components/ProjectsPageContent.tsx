...
export function ProjectsPageContent() {
    ...
    const allTechnologies = ["All", ...new Set(projects.flatMap(p => p.tags))];
...
function ProjectCard({ project, index, viewMode }: { project: IProject; index: number; viewMode: "grid" | "list" }) {
    if (viewMode === "list") {
        return (
            <motion.div
                ...
                className="group flex flex-col md:flex-row gap-6 p-6 rounded-xl border border-primary/20 bg-card/40 backdrop-blur-sm hover:border-primary/50 hover:shadow-lg transition-all"
            >
                <div className="md:w-64 aspect-video bg-muted/20 rounded-lg flex items-center justify-center overflow-hidden">
                    {project.image ? (
                ...
                <div className="flex-1 flex flex-col">
                    ...
                    <div className="flex items-center gap-4">
                        <Link href={project.github} target="_blank" className="inline-flex items-center text-sm font-medium hover:text-accent transition-colors">
                            <Github className="mr-2 h-4 w-4" />
                            Code
                        </Link>
                        {project.demo && (
                            <Link href={project.demo} target="_blank" className="inline-flex items-center text-sm font-medium hover:text-accent transition-colors">
                                <ExternalLink className="mr-2 h-4 w-4" />
                                Live Demo
                            </Link>
                        )}
                        <Link href={`/projects/${project._id}`} className="inline-flex items-center text-sm font-medium text-primary hover:text-primary/80 transition-colors ml-auto">
                            View Details
    ...
    return (
        <motion.div
            ...
            className="group relative rounded-xl border border-primary/20 bg-card/40 backdrop-blur-sm overflow-hidden flex flex-col hover:border-primary/50 hover:shadow-lg transition-all"
        >
